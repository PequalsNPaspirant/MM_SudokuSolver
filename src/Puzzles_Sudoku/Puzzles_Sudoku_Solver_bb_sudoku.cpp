...
#define MarkChanged(x) { unsigned char const *ucp = C2Grp[x]; ChangedGroup[*ucp] = ChangedGroup[*(ucp+1)] = ChangedGroup[*(ucp+2)] = Changed = 1; } 
	...
	**                                                              **
	...
	static void MakeGuess(void)
	{
		bb_sudoku_Matrix::m_countValuesTried += 1;
...
*/
	...
	unsigned long long bb_sudoku_Matrix::m_countCellsTried;
	unsigned long long bb_sudoku_Matrix::m_countValuesTried;

	bool bb_sudoku_Matrix::solve(const vector< vector<int> >& dataIn, vector< vector< vector<int> > >& solutionSetsOut, const unsigned int numSolutions, SudokuPuzzleBasicStats& stats)
	{
		bb_sudoku_Matrix obj(dataIn);
	...
	bb_sudoku_Matrix::bb_sudoku_Matrix(const vector< vector<int> >& dataIn)
		: m_isValid(true)
	{
		...
		bb_sudoku_Matrix::m_countCellsTried = 0;
		bb_sudoku_Matrix::m_countValuesTried = 0;

		const size_t size = dataIn.size();
		for (size_t row = 0; row < size; ++row)
			for (size_t column = 0; column < size; ++column)
					m_data[row * size + column] = dataIn[row][column];
	}

	bool bb_sudoku_Matrix::solve(vector< vector< vector<int> > >& solutionSetsOut, const unsigned int& numSolutions)
	{
		int solutions = bb_solver(m_data);
		vector< vector<int> > solution(9, vector<int>(9, 0));
		for (size_t row = 0; row < 9; ++row)
			for (size_t column = 0; column < 9; ++column)
				solution[row][column] = B2V[SolGrid[row * 9 + column]];
